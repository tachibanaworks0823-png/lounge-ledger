...
function renderCastManagement(){
  $('#castManagementTable').innerHTML=sortedCasts().map(c=>{const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';const address=[c.address,c.building].filter(Boolean).join(' ')||'—';return '<tr><td><b>'+c.name+'</b></td><td><span class="status">'+effectiveCastStatus(c)+'</span></td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td><td>'+castAge(c.birthday)+'</td><td>'+address+'</td><td><button class="text-button" onclick="copyCastAddress(\''+c.id+'\')">コピー</button></td><td>'+ (c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集・詳細</button></td></tr>';}).join('')||empty(8,'キャストを追加してください');
}
window.copyCastAddress=async id=>{const cast=data.casts.find(c=>c.id===id);const address=[cast?.address,cast?.building].filter(Boolean).join(' ');if(!address)return;try{await navigator.clipboard.writeText(address);alert('住所をコピーしました。');}catch(_error){const area=document.createElement('textarea');area.value=address;document.body.append(area);area.select();document.execCommand('copy');area.remove();alert('住所をコピーしました。');}};
