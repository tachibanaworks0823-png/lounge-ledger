...
function applyPayrollAdjustment(castId,values){
  ...
  const nominated=value('nominated',values.nominated),main=value('main',values.main),companion=value('companion',values.companion),hours=value('hours',values.hours),hourly=value('hourly',values.hourly),back=value('back',values.back);
  ...
  return {...values,nominated,main,companion,hours,hourly,back,deductions,advance:0,payout};
}
function calcCast(cast){
  ...
  return applyPayrollAdjustment(cast.id,{hours,advance,nominated,main,companion,drink,bottle,back,hourly,gross,deductions,payout:Math.max(0,gross-deductions-advance)});
}
