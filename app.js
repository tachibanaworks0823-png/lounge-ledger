...
function slipTableRow(s){
  ...
  return '<tr class="'+(unsettled?'unsettled-slip':'')+'"><td>'+dateJP(s.date)+'</td><td>'+ (s.id||'—')+'</td><td>'+ (s.customerName||'—')+'</td><td>'+ (s.guests? s.guests+'名':'—')+'</td><td>'+yen(s.total)+'</td><td>'+(s.receipt?'<input class="receipt-list-check" type="checkbox" checked disabled aria-label="領収証あり">':'')+'</td><td><span class="status '+(unsettled?'unsettled-status ':slipPaymentCashOnly(s)?'cash':'')+'">'+paymentLabel+'</span></td><td><button class="text-button" onclick="editSlip('+slipIndex+')">編集</button></td></tr>';
}
...
form.addEventListener('submit',e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const x=Object.fromEntries(new FormData(form));if(mode==='slip'){
  ...
  const record={id:x.id,date:x.date,customerName:x.customerName,total,card,receivable,payment,payments,receipt:Boolean(fields.querySelector('[name="receipt"]')?.checked),groups:1,guests:+x.guests,casts:[]};
