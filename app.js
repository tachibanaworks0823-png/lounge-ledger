...
function renderCastManagement(){
  $('#castManagementTable').innerHTML=sortedCasts().map(c=>'<tr><td><b>'+c.name+'</b></td><td><span class="status">'+effectiveCastStatus(c)+'</span></td><td>'+yen(c.hourly)+'</td><td>'+ (c.joinedDate?dateJP(c.joinedDate):'—')+'</td><td>'+ (c.phone||'—')+'</td><td>'+ (c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集・詳細</button></td></tr>').join('')||empty(7,'キャストを追加してください');
}
...
function renderShifts(){
  ...
  const counts='<tr class="shift-count-row"><th>出勤</th>'+days.map(d=>{const castCount=data.shifts.filter(x=>{const cast=data.casts.find(c=>c.id===x.castId);return x.date===d.date&&cast&&isEmployedOn(cast,d.date)&&!['×','当欠','無欠'].includes(String(x.schedule||'').trim());}).length,specialCount=data.shiftSpecials.filter(x=>x.date===d.date&&x.note).length;return '<td class="'+(d.shopClosed?'shop-closed ':'')+'">'+(d.shopClosed?'店休':castCount+'人 ('+specialCount+')')+'</td>';}).join('')+'</tr>';
  ...
  const scheduleCasts=visibleCasts.filter(c=>data.shifts.some(item=>item.castId===c.id&&isEmployedOn(c,item.date)));
