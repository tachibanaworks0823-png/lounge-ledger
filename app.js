...
function renderCastManagement(){
  const row=(c,showLeavingDate=false)=>{const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';return '<tr><td><b>'+c.name+'</b></td><td>'+castAge(c.birthday)+'</td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td>'+(showLeavingDate?'<td>'+(c.leavingDate?dateKey(c.leavingDate).replace(/-/g,'/'):'—')+'</td>':'')+'<td>'+ (c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集・詳細</button></td></tr>';};
  const casts=sortedCasts(),active=casts.filter(c=>effectiveCastStatus(c)!=='退店'),retired=casts.filter(c=>effectiveCastStatus(c)==='退店');
  $('#castManagementActiveTable').innerHTML=active.map(c=>row(c)).join('')||empty(5,'在籍キャストはいません');
...
function openForm(type,castId=null){mode=type;form.autocomplete=type==='cast'?'off':'on';fields.classList.toggle('cast-profile-fields',type==='cast');editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':(type==='shift'||type==='shiftBatch'||type==='shopClosed')?'SHIFT':'CAST';
 ...
 if(type==='application'){ $('#dialogTitle').textContent='応募を追加';fields.innerHTML=field('応募日','applicationDate','date')+'<label class="field">媒体<select name="media"><option value="" selected>選択してください</option>'+data.settings.applicationMedia.map(item=>'<option value="'+item+'">'+item+'</option>').join('')+'</select></label>'+optionalField('新しい媒体（必要なとき）','newMedia','text','full')+field('募集名','recruitmentName','text')+'<label class="field">生年月日<input name="birthday" type="date"></label>'+field('年齢','age','number')+field('連絡先','phone','tel')+field('Mail','email','email')+field('面接希望日','preferredInterviewDate','date')+field('面接確定日','confirmedInterviewDate','date')+field('時間','interviewTime','time')+'<label class="field">リスケ<select name="reschedule"><option value="" selected>選択してください</option>'+[1,2,3,4,5].map(item=>'<option value="'+item+'">'+item+'</option>').join('')+'</select></label>'+'<label class="field">ステータス<select name="status"><option value="" selected>選択してください</option><option>入店</option><option>不採用</option><option>返信無し</option><option>面接待ち</option><option>対応終了</option></select></label>'+'<label class="field full">備考<textarea name="note" rows="3"></textarea></label>';const birthday=fields.querySelector('[name="birthday"]'),age=fields.querySelector('[name="age"]');birthday.onchange=()=>{if(birthday.value)age.value=castAge(birthday.value).replace('歳','');};fields.querySelector('[name="applicationDate"]').value=businessDate();}
