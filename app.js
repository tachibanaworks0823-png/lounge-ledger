...
const batchBackDetail=()=>'<details class="batch-back-detail"><summary>詳細打込み（フリー・場内／本指名／同伴）</summary>'+backGroups.map(([title,keys],index)=>'<section class="back-detail-group group-'+index+'"><h4>'+title+'</h4><div>'+keys.map(key=>'<label>'+backInputLabels[key]+'<input class="batch-'+key+'" type="number" min="0"></label>').join('')+'</div></section>').join('')+'</details>';
