...
function batchCastRow(castId=''){const casts=sortedCasts().filter(c=>effectiveCastStatus(c)!=='退店');const options=casts.map(c=>'<option value="'+c.id+'"'+(c.id===castId?' selected':'')+'>'+c.name+'</option>').join('');return '<article class="daily-batch-row" data-cast-id="'+castId+'"><div class="batch-cast-head"><label><small>キャスト</small><select class="batch-cast-select">'+options+'</select></label><button type="button" class="text-button remove-batch-cast">削除</button></div><div><small>出勤</small>'+batchTimePicker('start')+'</div><div><small>退勤</small>'+batchTimePicker('end')+'</div><label><small>実働</small><output class="batch-hours"></output></label><label><small>日払い</small><input class="batch-advance" type="number"></label><label><small>引き物</small><input class="batch-deduction" type="number"></label><label class="batch-count-field"><small>場内</small><input class="batch-area" type="number"></label><label class="batch-count-field"><small>本指名</small><input class="batch-main" type="number"></label><label class="batch-count-field"><small>同伴</small><input class="batch-companion" type="number"></label><label><small>手当</small><input class="batch-allowance" type="number"></label><label><small>本指名売上</small><input class="batch-sales" type="number"></label>'+batchBackDetail()+'</article>';}
