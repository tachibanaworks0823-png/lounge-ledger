...
function renderExpenses(){const m={},expenses=data.expenses.filter(x=>expenseAccountingMonth(x)===data.month);expenses.forEach(x=>m[x.category]=(m[x.category]||0)+Number(x.amount));$('#expenseSummary').innerHTML=Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,4).map(x=>`<div class="category-card"><p>${x[0]}</p><strong>${yen(x[1])}</strong></div>`).join('')||'<div class="category-card"><p>支出を入力するとカテゴリ別に集計されます</p><strong>¥0</strong></div>';
$('#expenseTable').innerHTML=expenses.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(x=>`<tr><td>${dateJP(x.date)}</td><td>${yen(x.amount)}</td><td>${x.company}</td><td><span class="status">${x.category}</span></td><td>${x.note||'—'}</td><td><button class="text-button" onclick="editExpense('${x.id}')">編集</button><button class="text-button" onclick="removeItem('expenses','${x.id}')">削除</button></td></tr>`).join('')||empty(6,'支出はまだありません');}
