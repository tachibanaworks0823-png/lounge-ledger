...
function openForm(type){mode=type;$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 ...
 const now=new Date().toISOString().slice(0,10);fields.querySelectorAll('input[type=date]').forEach(x=>x.value=now);if(type==='slip')fields.elements.date.value=businessDate();showEntryDialog();
}
['addSlip','dashboardAddSlip'].forEach(id=>$('#'+id).onclick=e=>{e.preventDefault();openForm('slip')});
document.addEventListener('click',e=>{if(e.target.closest('#addSlip,#dashboardAddSlip')){e.preventDefault();openForm('slip')}});$('#addExpense').onclick=()=>openForm('expense');$('#addShift').onclick=()=>openForm('shift');$('#addCast').onclick=()=>openForm('cast');
