...
function renderShifts(){
  ...
  $('#shiftTableHead').innerHTML='<tr><th class="shift-name-head">キャスト</th>'+days.map(d=>'<th class="shift-day-head '+(d.shopClosed?'shop-closed ':d.holiday?'holiday ':d.weekdayIndex===0?'sunday':d.weekdayIndex===6?'saturday':'')+'">'+d.day+'<small>('+d.weekday+')</small>'+((d.holiday||d.shopClosed)?'<em>'+[d.holiday,d.shopClosed?'店休':''].filter(Boolean).join('・')+'</em>':'')+'</th>').join('')+'</tr>';
