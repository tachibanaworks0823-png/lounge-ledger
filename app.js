...
function renderCastManagement(){
  const row=(c,showLeavingDate=false,showAddress=false)=>{const profileAge=(c.age!==''&&c.age!==undefined&&c.age!==null)?String(c.age)+'歳':castAge(c.birthday);const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';const address=[c.address,c.building].filter(Boolean).join(' ')||'—';const memo='<td>'+(c.memo||'—')+'</td>';const addressCells='<td><button class="text-button" onclick="copyCastAddress(&quot;'+c.id+'&quot;)">コピー</button></td><td>'+address+'</td>';const actions='<td><button class="text-button" onclick="editCastProfile(&quot;'+c.id+'&quot;)">編集・詳細</button></td>';const nameCell='<td><b>'+c.name+'</b><button class="cast-inline-edit" type="button" onclick="editCastProfile(&quot;'+c.id+'&quot;)">編集・詳細</button></td>';return '<tr>'+nameCell+'<td>'+profileAge+'</td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td>'+(showLeavingDate?'<td>'+(c.leavingDate?dateKey(c.leavingDate).replace(/-/g,'/'):'—')+'</td>':'')+(showAddress?addressCells+memo:memo)+actions+'</tr>';};
  const hiddenRow=c=>'<tr><td><b>'+c.name+'</b><button class="cast-inline-edit" type="button" onclick="editCastProfile(&quot;'+c.id+'&quot;)">編集・詳細</button></td><td>'+effectiveCastStatus(c)+'</td><td>'+((c.age!==''&&c.age!==undefined&&c.age!==null)?String(c.age)+'歳':castAge(c.birthday))+'</td><td>'+(c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(&quot;'+c.id+'&quot;)">編集・詳細</button></td></tr>';
