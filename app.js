...
function applyPayrollAdjustment(castId,values){
  ...
  const nominated=value('nominated',values.nominated),area=value('area',values.area),main=value('main',values.main),companion=value('companion',values.companion),hours=value('hours',values.hours),hourly=value('hourly',values.hourly),back=value('back',values.back);
  const deductions=value('deductions',Number(values.deductions||0)+Number(values.advance||0));
...
function renderCasts(){
  ...
  $('#castTable').innerHTML=sortedCasts().filter(visibleInPayroll).map(c=>{
    ...
    return '<tr><td><b>'+c.name+'</b><br><small>時給 '+yen(c.hourly)+'</small></td><td>'+value(x.nominated)+'</td><td>'+count(x.area,'本')+' / '+count(x.main,'本')+' / '+count(x.companion,'本')+' / '+count(x.extension,'本')+'</td><td>'+count(x.days,'日')+'</td><td>'+Number(x.hours||0).toFixed(1)+'h</td><td>'+value(x.hourly)+'</td><td>'+count(x.companionBack,'本')+'</td><td>'+value(x.mainBack)+'</td><td>'+count(x.extensionBack,'本')+'</td><td>'+count(x.drink,'杯')+'</td><td>'+count(x.decoration,'個')+'</td><td>'+count(x.bottleChampagne,'本')+'</td><td>'+value(x.back)+'</td><td>'+value(x.consumptionTax)+'</td><td>'+value(x.incomeTax)+'</td><td>'+value(x.welfare)+'</td><td>'+value(x.pull)+'</td><td>'+value(x.deductionTotal)+'</td><td>'+value(x.advanceAmount)+'</td><td>'+value(x.gross)+'</td><td>'+value(x.payoutBeforeAdvance)+'</td><td><b>'+value(x.payout)+'</b></td><td>'+payRate+'</td><td>'+averageHourly+'</td><td><button class="text-button" onclick="editCastPayroll(\''+c.id+'\')">詳細・編集</button></td></tr>';
