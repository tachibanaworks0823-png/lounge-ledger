...
function renderShifts(){
  ...
  const counts='<tr class="shift-count-row"><th>出勤</th>'+days.map(d=>{const castCount=data.shifts.filter(x=>x.date===d.date&&!['×','当欠','無欠'].includes(String(x.schedule||'').trim())).length,specialCount=data.shiftSpecials.filter(x=>x.date===d.date&&x.note).length;return '<td class="'+(d.shopClosed?'shop-closed ':'')+'">'+(d.shopClosed?'店休':castCount+'人 ('+specialCount+')')+'</td>';}).join('')+'</tr>';
