...
function renderDailyInputs(){const grouped=new Map();data.dailyInputs.forEach(x=>{if(!grouped.has(x.date))grouped.set(x.date,[]);grouped.get(x.date).push(x);});const dates=[...grouped.keys()].sort((a,b)=>b.localeCompare(a));$('#dailyInputSummary').textContent=data.month.replace('-','年')+'月・'+data.dailyInputs.length+'件';$('#dailyInputTable').innerHTML=dates.map(date=>'<tr><td><b>'+dateJP(date)+'</b></td><td>'+grouped.get(date).length+'人分</td><td><button class="primary-button compact-button" onclick="openDailyDateDetails(\''+date+'\')">詳細を見る</button></td></tr>').join('')||empty(3,'日別打込みはまだありません');}
...
window.editCastProfile=id=>openForm('cast',id);window.editDailyInput=id=>{if(dialog.open)closeEntryDialog();openForm('dailyInput',id);};window.openDailyDateDetails=date=>openForm('dailyDetails',date);
...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 ...
 if(type==='dailyBatch'){ $('#dialogTitle').textContent='日別まとめ入力';const first=sortedCasts().find(c=>c.status!=='退店')?.id||'';fields.innerHTML=field('日付','date','date','full')+'<div class="batch-toolbar full"><span>キャストを選択して入力してください</span><button type="button" class="secondary-button" onclick="addBatchCastRow()">＋ キャストを追加</button></div><div class="daily-batch-list full" id="batchCastList">'+batchCastRow(first)+'</div>'; }
...
$('#sortSlipsDate').onclick=()=>{slipDateSort=slipDateSort==='asc'?'desc':'asc';renderSlips()};$('#addDailyInput').onclick=()=>openForm('dailyBatch');$('#addExpense').onclick=()=>openForm('expense');$('#addShift').onclick=()=>openForm('shift');$('#addShiftBatch').onclick=()=>openForm('shiftBatch');$('#addCast').onclick=()=>openForm('cast');$('#addCastProfile').onclick=()=>openForm('cast');
