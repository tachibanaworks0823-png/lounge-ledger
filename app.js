...
function renderDailyInputs(){const [year,month]=data.month.split('-').map(Number),count=new Date(year,month,0).getDate(),grouped=new Map(),statuses=new Map();data.dailyInputs.forEach(x=>{if(!grouped.has(x.date))grouped.set(x.date,[]);grouped.get(x.date).push(x);});data.dailyStatuses.forEach(x=>statuses.set(x.date,x.status));const dates=Array.from({length:count},(_,i)=>data.month+'-'+String(i+1).padStart(2,'0')).sort((a,b)=>a.localeCompare(b)*(dailyInputDateSort==='asc'?1:-1));$('#dailyInputSummary').textContent=data.month.replace('-','年')+'月・'+data.dailyInputs.length+'件';$('#sortDailyInputDate').textContent='日付 '+(dailyInputDateSort==='asc'?'↑':'↓');const options=status=>['営業','店休','キャスト0'].map(value=>'<option'+(status===value?' selected':'')+'>'+value+'</option>').join('');$('#dailyInputTable').innerHTML=dates.map(date=>{const entries=grouped.get(date)||[],status=statuses.get(date)||'営業';return '<tr><td><b>'+dateJP(date)+'</b></td><td><select class="daily-status-select" onchange="updateDailyStatus(\''+date+'\',this.value)">'+options(status)+'</select></td><td>'+entries.length+'人分</td><td><button class="primary-button compact-button" onclick="openDailyDateDetails(\''+date+'\')">詳細を見る</button></td></tr>';}).join('');}
...
function renderShifts(){
  ...
  const counts='<tr class="shift-count-row"><th>出勤</th>'+days.map(d=>{const castCount=data.shifts.filter(x=>x.date===d.date).length,specialCount=data.shiftSpecials.filter(x=>x.date===d.date&&x.note).length;return '<td>'+castCount+'人 ('+specialCount+')</td>';}).join('')+'</tr>';
  const specialRows=[['interview','面接・体入'],['trial','面接・体入']].map(([type,label])=>'<tr class="shift-special-row '+type+'"><th>'+label+'</th>'+days.map(d=>{const item=data.shiftSpecials.find(x=>x.type===type&&x.date===d.date);const value=item?.note||'';return '<td><button type="button" onclick="editShiftSpecial(&quot;'+type+'&quot;,&quot;'+d.date+'&quot;)">'+value+'</button></td>';}).join('')+'</tr>').join('');
...
function batchCastRow(castId=''){const casts=sortedCasts().filter(c=>c.status!=='退店');const options=casts.map(c=>'<option value="'+c.id+'"'+(c.id===castId?' selected':'')+'>'+c.name+'</option>').join('');return '<article class="daily-batch-row" data-cast-id="'+castId+'"><div class="batch-cast-head"><label><small>キャスト</small><select class="batch-cast-select">'+options+'</select></label><button type="button" class="text-button remove-batch-cast">削除</button></div><div><small>出勤</small>'+batchTimePicker('start')+'</div><div><small>退勤</small>'+batchTimePicker('end')+'</div><label><small>実働</small><output class="batch-hours"></output></label><label><small>日払い</small><input class="batch-advance" type="number"></label><label><small>引き物</small><input class="batch-deduction" type="number"></label><label><small>場内</small><input class="batch-area" type="number"></label><label><small>本指名</small><input class="batch-main" type="number"></label><label><small>同伴</small><input class="batch-companion" type="number"></label><label><small>本指名売上</small><input class="batch-sales" type="number"></label>'+batchBackDetail()+'</article>';}
...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 if(type==='slip'){
 ...
 if(type==='shiftBatch'){ $('#dialogTitle').textContent='シフトをまとめて入力';const casts=sortedCasts().filter(c=>c.status!=='退店');fields.innerHTML=field('勤務日','date','date','full')+'<div class="shift-batch-list full">'+casts.map(c=>'<label class="shift-batch-row" data-cast-id="'+c.id+'"><b>'+c.name+'</b><input class="shift-batch-value" placeholder="例：8、8-12.5、×、休み"></label>').join('')+'</div>'; }
