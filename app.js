...
function renderSlips(){
  ...
  $('#slipTable').innerHTML=slips.slice().sort((a,b)=>{const dateOrder=a.date.localeCompare(b.date)*direction;return dateOrder||String(a.id||'').localeCompare(String(b.id||''),'ja',{numeric:true})*direction;}).map(s=>{
    const payment=slipPaymentSummary(s);
    const slipIndex=data.slips.indexOf(s);return '<tr><td>'+dateJP(s.date)+'</td><td>'+ (s.id||'—')+'</td><td>'+ (s.customerName||'—')+'</td><td>'+ (s.guests? s.guests+'名':'—')+'</td><td>'+yen(s.total)+'</td><td><span class="status '+(slipPaymentCashOnly(s)?'cash':'')+'">'+payment+'</span></td><td><button class="text-button" onclick="editSlip('+slipIndex+')">編集</button></td></tr>';
...
function openForm(type,castId=null){mode=type;form.autocomplete=(type==='cast'||type==='application')?'off':'on';fields.classList.toggle('cast-profile-fields',type==='cast');editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;editingApplicationId=type==='application'?castId:null;editingSlipIndex=type==='slip'&&castId!==null?Number(castId):null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#deleteApplicationButton').hidden=!(type==='application'&&editingApplicationId);$('#deleteSlipButton').hidden=!(type==='slip'&&editingSlipIndex!==null);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':(type==='shift'||type==='shiftBatch'||type==='shopClosed')?'SHIFT':'CAST';
 if(type==='slip'){
   ...
   '<label class="field full customer-history-picker-field">顧客名の履歴<select id="customerHistoryPicker"><option value="">履歴から選択</option>'+customerNames().map(name=>'<option value="'+String(name).replace(/"/g,'&quot;')+'">'+name+'</option>').join('')+'</select></label>'+
   ...
   const customerHistoryPicker=fields.querySelector('#customerHistoryPicker');
   if(customerHistoryPicker)customerHistoryPicker.onchange=()=>{if(customerHistoryPicker.value)fields.querySelector('[name="customerName"]').value=customerHistoryPicker.value;};
