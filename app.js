...
function renderShifts(){
  ...
  const days=Array.from({length:count},(_,i)=>{const day=i+1,date=data.month+'-'+String(day).padStart(2,'0'),weekdayIndex=new Date(date+'T12:00:00').getDay();return {day,date,weekday:weekdays[weekdayIndex],weekdayIndex,holiday:holidays[date]||'',shopClosed:data.dailyStatuses.some(item=>item.date===date&&item.status==='店休')};});
  $('#shiftMonthTitle').textContent=year+'年 '+month+'月 シフト表';$('.shift-table').style.setProperty('--shift-days',count);
  $('#shiftTableHead').innerHTML='<tr><th class="shift-name-head">キャスト</th>'+days.map(d=>'<th class="shift-day-head '+(d.shopClosed?'shop-closed ':d.holiday?'holiday ':d.weekdayIndex===0?'sunday':d.weekdayIndex===6?'saturday':'')+'">'+d.day+'<small>('+d.weekday+')</small>'+((d.shopClosed||d.holiday)?'<em>'+(d.shopClosed?'店休':d.holiday)+'</em>':'')+'</th>').join('')+'</tr>';
  const counts='<tr class="shift-count-row"><th>出勤</th>'+days.map(d=>{const castCount=data.shifts.filter(x=>x.date===d.date).length,specialCount=data.shiftSpecials.filter(x=>x.date===d.date&&x.note).length;return '<td class="'+(d.shopClosed?'shop-closed ':'')+'">'+(d.shopClosed?'店休':castCount+'人 ('+specialCount+')')+'</td>';}).join('')+'</tr>';
  ...
  const castRows=sortedCasts().map(c=>'<tr><th class="shift-cast-name">'+c.name+'</th>'+days.map(d=>{const shift=data.shifts.find(x=>x.castId===c.id&&x.date===d.date);const label=shift?(shift.schedule||shift.hours+'h'):'';const cls=(shift?'has-shift ':'empty-shift ')+(d.shopClosed?'shop-closed ':d.holiday?'holiday ':d.weekdayIndex===0?'sunday':d.weekdayIndex===6?'saturday':'');return '<td class="'+cls+'"><button type="button" onclick="editShiftCell(&quot;'+c.id+'&quot;,&quot;'+d.date+'&quot;)">'+(d.shopClosed?'':label)+'</button></td>';}).join('')+'</tr>').join('');
...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':(type==='shift'||type==='shiftBatch'||type==='shopClosed')?'SHIFT':'CAST';
 ...
 if(type==='shiftBatch'){ $('#dialogTitle').textContent='キャスト別シフトを一括入力';const casts=sortedCasts().filter(c=>c.status!=='退店');fields.innerHTML='<label class="field full">キャスト<select name="castId">'+casts.map(c=>'<option value="'+c.id+'">'+c.name+'</option>').join('')+'</select></label><div class="shift-batch-list full"></div>';fields.querySelector('[name="castId"]').onchange=renderShiftBatchRows;renderShiftBatchRows(); }
 if(type==='shopClosed'){ $('#dialogTitle').textContent='店休を設定';fields.innerHTML=field('店休日','date','date','full');}
