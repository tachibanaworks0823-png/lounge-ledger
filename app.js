...
function renderCastManagement(){
  $('#castManagementTable').innerHTML=sortedCasts().map(c=>{const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';return '<tr><td><b>'+c.name+'</b></td><td><span class="status">'+effectiveCastStatus(c)+'</span></td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td><td>'+yen(c.hourly)+'</td><td>'+ (c.joinedDate?dateJP(c.joinedDate):'—')+'</td><td>'+ (c.phone||'—')+'</td><td>'+ (c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集・詳細</button></td></tr>';}).join('')||empty(8,'キャストを追加してください');
}
