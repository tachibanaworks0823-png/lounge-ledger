...
function openForm(type,castId=null){mode=type;const primarySave=form.querySelector('.primary-button[value="save"]');primarySave.type='submit';primarySave.onclick=null;form.autocomplete=(type==='cast'||type==='application')?'off':'on';fields.classList.toggle('cast-profile-fields',type==='cast');fields.classList.toggle('daily-input-edit-fields',type==='dailyInput');editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;editingApplicationId=type==='application'?castId:null;editingSlipIndex=type==='slip'&&castId!==null?Number(castId):null;editingExpenseId=type==='expense'?castId:null;editingPayrollCastId=(type==='payroll'||type==='payrollDetails')?castId:null;form.querySelector('[value="save"]').hidden=(type==='dailyDetails'||type==='payrollDetails');$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);const castVisibilityButton=$('#toggleCastVisibilityButton');castVisibilityButton.hidden=!(type==='cast'&&editingCastId);if(type==='cast'&&editingCastId){const cast=data.casts.find(c=>c.id===editingCastId);castVisibilityButton.textContent=cast?.hidden?'表示に戻す':'非表示';}$('#deleteApplicationButton').hidden=!(type==='application'&&editingApplicationId);$('#deleteSlipButton').hidden=!(type==='slip'&&editingSlipIndex!==null);$('#deleteExpenseButton').hidden=!(type==='expense'&&editingExpenseId);$('#dialogKicker').textContent='';
 ...
 if(type==='dailyBatch'){ $('#dialogTitle').textContent='日別まとめ入力';const first=sortedCasts().find(c=>!c.hidden&&effectiveCastStatus(c)!=='退店')?.id||'';fields.innerHTML=field('日付','date','date')+'<div class="batch-toolbar full"><span>キャストを選択して入力してください</span><button type="button" class="secondary-button" onclick="addBatchCastRow()">＋ キャストを追加</button></div><div class="daily-batch-list full" id="batchCastList">'+batchCastRow(first)+'</div>'; }
