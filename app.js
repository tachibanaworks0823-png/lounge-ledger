...
function render(){ renderDashboard();renderSlips();renderCasts();renderCastManagement();renderShifts();renderExpenses();renderSettings(); }
function dailyRows(){
...
function renderCastManagement(){
  $('#castManagementTable').innerHTML=data.casts.map(c=>'<tr><td><b>'+c.name+'</b></td><td><span class="status">'+(c.status||'在籍')+'</span></td><td>'+yen(c.hourly)+'</td><td>'+ (c.joinedDate?dateJP(c.joinedDate):'—')+'</td><td>'+ (c.phone||'—')+'</td><td>'+ (c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集</button></td></tr>').join('')||empty(7,'キャストを追加してください');
}
...
const dialog=$('#entryDialog'), form=$('#entryForm'), fields=$('#formFields');let mode='', slipDateSort='desc', editingCastId=null;
...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 if(type==='slip'){
 ...
 if(type==='cast'){ const cast=data.casts.find(c=>c.id===editingCastId);$('#dialogTitle').textContent=cast?'キャストを編集':'キャストを追加';fields.innerHTML=field('お名前','name')+'<label class="field">在籍状況<select name="status"><option>在籍</option><option>体入</option><option>休職</option><option>退店</option></select></label>'+field('基本時給','hourly','number')+field('入店日','joinedDate','date')+field('連絡先','phone','tel')+field('誕生日','birthday','date')+'<label class="field full">メモ<textarea name="memo" rows="3"></textarea></label>';if(cast){fields.elements.name.value=cast.name||'';fields.elements.status.value=cast.status||'在籍';fields.elements.hourly.value=cast.hourly||'';fields.elements.joinedDate.value=cast.joinedDate||'';fields.elements.phone.value=cast.phone||'';fields.elements.birthday.value=cast.birthday||'';fields.elements.memo.value=cast.memo||'';}}
 const now=new Date().toISOString().slice(0,10);if(!(type==='cast'&&editingCastId))fields.querySelectorAll('input[type=date]').forEach(x=>x.value=now);if(type==='slip'){const dateField=fields.querySelector('input[name="date"]');if(dateField)dateField.value=businessDate();}showEntryDialog();
}
['addSlip','dashboardAddSlip'].forEach(id=>$('#'+id).onclick=e=>{e.preventDefault();openForm('slip')});
$('#sortSlipsDate').onclick=()=>{slipDateSort=slipDateSort==='asc'?'desc':'asc';renderSlips()};$('#addExpense').onclick=()=>openForm('expense');$('#addShift').onclick=()=>openForm('shift');$('#addShiftFromSchedule').onclick=()=>openForm('shift');$('#addCast').onclick=()=>openForm('cast');$('#addCastProfile').onclick=()=>openForm('cast');
form.addEventListener('submit',e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const x=Object.fromEntries(new FormData(form));if(mode==='slip'){data.slips.push({id:x.id,date:x.date,customerName:x.customerName,total:+x.total,card:x.payment==='カード'?+x.total:0,payment:x.payment,groups:1,guests:+x.guests,nominationType:x.nominationType,casts:[]})}if(mode==='expense'){const category=(x.newCategory||'').trim()||x.category;if((x.newCategory||'').trim()&&!data.settings.categories.includes(category))data.settings.categories.push(category);if(!data.settings.payeeHistory.includes(x.company))data.settings.payeeHistory.push(x.company);data.expenses.push({id:'E-'+Date.now(),date:x.date,category,company:x.company,note:x.note,amount:+x.amount})}if(mode==='shift'){data.shifts.push({date:x.date,castId:x.castId,hours:+x.hours,advance:+x.advance})}if(mode==='cast'){const profile={name:x.name,hourly:+x.hourly,status:x.status,joinedDate:x.joinedDate,phone:x.phone,birthday:x.birthday,memo:x.memo};const existing=data.casts.find(c=>c.id===editingCastId);if(existing)Object.assign(existing,profile);else data.casts.push({id:'c-'+Date.now(),...profile})}save();render();dialog.close();});
