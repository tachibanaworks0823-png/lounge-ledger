...
function renderSlips(){
  ...
  $('#slipTable').innerHTML=slips.slice().sort((a,b)=>{const dateOrder=a.date.localeCompare(b.date)*direction;return dateOrder||String(a.id||'').localeCompare(String(b.id||''),'ja',{numeric:true})*direction;}).map(s=>{
    ...
    const slipIndex=data.slips.indexOf(s);return '<tr><td>'+dateJP(s.date)+'</td><td>'+ (s.id||'—')+'</td><td>'+ (s.customerName||'—')+'</td><td>'+ (s.guests? s.guests+'名':'—')+'</td><td>'+yen(s.total)+'</td><td><span class="status '+(paymentMethodCategory(payment)==='card'?'':'cash')+'">'+payment+'</span></td><td><button class="text-button" onclick="editSlip('+slipIndex+')">編集</button><button class="text-button" onclick="removeItem(\'slips\',\''+s.id+'\')">削除</button></td></tr>';
...
const dialog=$('#entryDialog'), form=$('#entryForm'), fields=$('#formFields'), mediaOrderDialog=$('#mediaOrderDialog'), mediaOrderList=$('#mediaOrderList'), paymentMethodDialog=$('#paymentMethodDialog'), paymentMethodForm=$('#paymentMethodForm'), paymentMethodList=$('#paymentMethodList'), customerHistoryDialog=$('#customerHistoryDialog'), customerHistoryList=$('#customerHistoryList');let mode='', slipDateSort='desc', dailyInputDateSort='desc', editingCastId=null, editingDailyInputId=null, editingApplicationId=null, editingSlipIndex=null, mediaOrderDraft=[];
