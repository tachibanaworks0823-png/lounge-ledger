...
function bindDesktopTimePickers(){fields.querySelectorAll('.desktop-time-picker').forEach(picker=>{const hour=picker.querySelector('.time-hour'),minute=picker.querySelector('.time-minute'),target=picker.querySelector('input[type=hidden]');const update=()=>{target.value=hour.value&&minute.value?hour.value+':'+minute.value:'';calculateWorkHours();};hour.onchange=update;minute.onchange=update;});}
