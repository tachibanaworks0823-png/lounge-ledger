...
function renderCasts(){
  ...
  $('#castTable').innerHTML=sortedCasts().filter(visibleInPayroll).map(c=>{
    ...
    return '<tr><td><b>'+c.name+'</b><br><small>時給 '+yen(c.hourly)+'</small></td><td>'+value(x.nominated)+'</td><td>'+count(x.area,'本')+' / '+count(x.main,'本')+' / '+count(x.companion,'本')+' / '+count(x.extension,'本')+'</td><td>'+count(x.days,'日')+'</td><td>'+Number(x.hours||0).toFixed(1)+'h</td><td>'+value(x.hourly)+'</td><td>'+value(x.companionBack)+'</td><td>'+value(x.mainBack)+'</td><td>'+value(x.extensionBack)+'</td><td>'+value(x.drink)+'</td><td>'+value(x.decoration)+'</td><td>'+value(x.bottleChampagne)+'</td><td>'+value(x.allowance)+'</td><td>'+value(Number(x.back||0)+Number(x.allowance||0))+'</td><td>'+value(x.consumptionTax)+'</td><td>'+value(x.incomeTax)+'</td><td>'+value(x.welfare)+'</td><td>'+value(x.pull)+'</td><td>'+value(x.deductionTotal)+'</td><td>'+value(x.advanceAmount)+'</td><td>'+value(x.gross)+'</td><td>'+value(x.payoutBeforeAdvance)+'</td><td><b>'+value(x.payout)+'</b></td><td>'+payRate+'</td><td>'+averageHourly+'</td><td><button class="text-button" onclick="editCastPayroll(\''+c.id+'\')">詳細・編集</button></td></tr>';
...
function renderCastManagement(){
  ...
  const casts=sortedCasts(),hidden=casts.filter(c=>c.hidden),active=casts.filter(c=>!c.hidden&&effectiveCastStatus(c)!=='退店'),retired=casts.filter(c=>!c.hidden&&effectiveCastStatus(c)==='退店');
...
const batchTimePicker=kind=>useNativeBatchTime()?'<div class="batch-time"><input class="batch-'+kind+'-time batch-native-time" type="time" step="1800"></div>':'<div class="batch-time"><select class="batch-'+kind+'-hour"><option value="">--</option>'+Array.from({length:12},(_,i)=>'<option value="'+String(i+1)+'">'+String(i+1)+'</option>').join('')+'</select><span>:</span><select class="batch-'+kind+'-minute"><option value="">--</option><option value="00">00</option><option value="30">30</option></select></div>';
...
const backGroups=[['フリー・場内',['free1000','free1500','free2000','free2500','free3000']],['本指名',['main1000','main1500','main2000','main2500','main3000','mainP','mainDecoration','mainBottle','mainChampagne','mainExtension']],['同伴',['companion1000','companion1500','companion2000','companion2500','companion3000','companionP','companionDecoration','companionBottle','companionChampagne','companionExtension']]];
const batchBackDetail=()=>'<details class="batch-back-detail"><summary>バック打込み</summary>'+backGroups.map(([title,keys],index)=>'<section class="back-detail-group group-'+index+'"><h4>'+title+'</h4><div>'+keys.map(key=>'<label>'+backInputLabels[key]+'<input class="batch-'+key+'" type="number" min="0"></label>').join('')+'</div></section>').join('')+'</details>';
const dailyBackDetail=()=>'<details class="daily-back-detail full"><summary>詳細打込み（フリー・場内／本指名／同伴）</summary>'+backGroups.map(([title,keys],index)=>'<section class="back-detail-group group-'+index+'"><h4>'+title+'</h4><div>'+keys.map(key=>'<label>'+backInputLabels[key]+'<input name="'+key+'" type="number" min="0"></label>').join('')+'</div></section>').join('')+'</details>';
function batchCastRow(castId=''){const casts=sortedCasts().filter(c=>effectiveCastStatus(c)!=='退店');const options=casts.map(c=>'<option value="'+c.id+'"'+(c.id===castId?' selected':'')+'>'+c.name+'</option>').join('');return '<article class="daily-batch-row" data-cast-id="'+castId+'"><div class="batch-cast-head"><label><small>キャスト</small><select class="batch-cast-select">'+options+'</select></label><button type="button" class="text-button remove-batch-cast">リセット</button></div><div class="batch-time-field"><small>出勤</small>'+batchTimePicker('start')+'</div><div class="batch-time-field"><small>退勤</small>'+batchTimePicker('end')+'</div><label class="batch-work-field"><small>実働</small><output class="batch-hours"></output></label><label class="batch-count-field"><small>場内</small><input class="batch-area" type="number"></label><label class="batch-count-field"><small>本指名</small><input class="batch-main" type="number"></label><label class="batch-count-field"><small>同伴</small><input class="batch-companion" type="number"></label><label class="batch-work-field"><small>日払い</small><input class="batch-advance" type="number"></label><label class="batch-count-field"><small>手当</small><input class="batch-allowance" type="number"></label><label class="batch-work-field"><small>引き物</small><input class="batch-deduction" type="number"></label><label class="batch-sales-field"><small>本指名売上</small><input class="batch-sales" type="number"></label>'+batchBackDetail()+'</article>';}
...
function openForm(type,castId=null){mode=type;const primarySave=form.querySelector('.primary-button[value="save"]');primarySave.type='submit';primarySave.onclick=null;form.autocomplete=(type==='cast'||type==='application')?'off':'on';fields.classList.toggle('cast-profile-fields',type==='cast');fields.classList.toggle('daily-input-edit-fields',type==='dailyInput');editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;editingApplicationId=type==='application'?castId:null;editingSlipIndex=type==='slip'&&castId!==null?Number(castId):null;editingExpenseId=type==='expense'?castId:null;editingPayrollCastId=type==='payroll'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);const castVisibilityButton=$('#toggleCastVisibilityButton');castVisibilityButton.hidden=!(type==='cast'&&editingCastId);if(type==='cast'&&editingCastId){const cast=data.casts.find(c=>c.id===editingCastId);castVisibilityButton.textContent=cast?.hidden?'表示に戻す':'非表示';}$('#deleteApplicationButton').hidden=!(type==='application'&&editingApplicationId);$('#deleteSlipButton').hidden=!(type==='slip'&&editingSlipIndex!==null);$('#deleteExpenseButton').hidden=!(type==='expense'&&editingExpenseId);$('#dialogKicker').textContent='';
 if(type==='payroll'){
 ...
 if(type==='dailyDetails'){const date=castId,entries=data.dailyInputs.filter(x=>x.date===date);const metric=(label,value)=>'<div class="daily-detail-metric"><span>'+label+'</span><strong>'+value+'</strong></div>';$('#dialogTitle').textContent=dateJP(date)+' の詳細';fields.innerHTML='<div class="daily-detail-list full">'+entries.map(x=>{const calc=calcDailyInput(x),nominationCount=Number(x.mainCount||0)+Number(x.companionCount||0);return '<article><div class="daily-detail-head"><b>'+castName(x.castId)+'</b><button type="button" class="text-button" onclick="editDailyInput(\''+x.id+'\')">詳細・編集</button></div><div class="daily-detail-grid">'+metric('実働',calc.hours?calc.hours+'h':'—')+metric('日払い',yen(calc.advance))+metric('引き物',yen(calc.deduction))+metric('同伴B',yen(calc.companionBack))+metric('本指名B',yen(calc.mainBack))+metric('延長B',yen(calc.extensionBack))+metric('ドリンク',yen(calc.drink))+metric('ボトル',yen(calc.bottleChampagne))+metric('飾り物',yen(calc.decoration))+metric('指名本数',nominationCount+'本')+metric('売上',yen(x.mainSales))+'</div></article>';}).join('')+'</div>'; }
 if(type==='dailyInput'){ const entry=data.dailyInputs.find(x=>x.id===editingDailyInputId);$('#dialogTitle').textContent=entry?'日別打込みを編集':'日別打込み';fields.innerHTML=field('日付','date','date','daily-edit-full')+'<label class="field daily-edit-full">キャスト<select name="castId">'+sortedCasts().map(c=>'<option value="'+c.id+'">'+c.name+'</option>').join('')+'</select></label>'+timeField('出勤','startTime','daily-edit-third')+timeField('退勤','endTime','daily-edit-third')+optionalField('実働時間','hours','number','auto-hours daily-edit-third')+optionalField('日払い','advance','number','daily-edit-third')+optionalField('引き物','deduction','number','daily-edit-third')+optionalField('手当','allowance','number','daily-edit-third')+optionalField('場内指名 本数','areaNomination','number','daily-edit-third')+optionalField('本指名 本数','mainCount','number','daily-edit-third')+optionalField('同伴 本数','companionCount','number','daily-edit-third')+optionalField('本指名 売上','mainSales','number','daily-edit-full')+dailyBackDetail();if(entry)['date','castId','startTime','endTime','hours','advance','deduction','areaNomination','mainCount','allowance','companionCount','mainSales',...backInputKeys].forEach(name=>{const input=fields.querySelector('[name="'+name+'"]');if(input)input.value=(name==='startTime'||name==='endTime')?normalizeDailyTime(entry[name]):(entry[name]??'');});}
