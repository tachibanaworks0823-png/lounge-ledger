...
function renderCastManagement(){
  const row=c=>{const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';return '<tr><td><b>'+c.name+'</b></td><td>'+castAge(c.birthday)+'</td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td><td>'+ (c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集・詳細</button></td></tr>';};
  const casts=sortedCasts(),active=casts.filter(c=>effectiveCastStatus(c)!=='退店'),retired=casts.filter(c=>effectiveCastStatus(c)==='退店');
  $('#castManagementActiveTable').innerHTML=active.map(row).join('')||empty(5,'在籍キャストはいません');
  $('#castManagementRetiredTable').innerHTML=retired.map(row).join('')||empty(5,'退店キャストはいません');
}
