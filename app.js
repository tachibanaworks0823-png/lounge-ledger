...
function renderShifts(){
  ...
  const counts='<tr class="shift-count-row"><th>出勤</th>'+days.map(d=>{const castCount=data.shifts.filter(x=>{const cast=data.casts.find(c=>c.id===x.castId);return x.date===d.date&&cast&&isEmployedOn(cast,d.date)&&!/[×✕☓]/.test(String(x.schedule||''))&&!String(x.schedule||'').includes('当欠')&&!String(x.schedule||'').includes('無欠')&&!String(x.schedule||'').includes('休');}).length,specialCount=data.shiftSpecials.filter(x=>x.date===d.date&&x.note).length;return '<td class="'+(d.shopClosed?'shop-closed ':'')+'">'+(d.shopClosed?'店休':castCount+'人 ('+specialCount+')')+'</td>';}).join('')+'</tr>';
