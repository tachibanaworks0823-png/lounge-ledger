...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 ...
 if(type==='cast'){ const cast=data.casts.find(c=>c.id===editingCastId);$('#dialogTitle').textContent=cast?'キャストを編集・詳細':'キャストを追加';fields.innerHTML=field('お名前','name')+'<label class="field">在籍状況<select name="status"><option>在籍</option><option>体入</option><option>休職</option><option>退店</option></select></label>'+optionalField('連絡先','phone','tel')+optionalField('入店日','joinedDate','date')+optionalField('基本時給','hourly','number')+optionalField('退店日','leavingDate','date')+optionalField('誕生日','birthday','date')+'<label class="field full">メモ<textarea name="memo" rows="3"></textarea></label>';if(cast){['name','status','hourly','joinedDate','leavingDate','phone','birthday','memo'].forEach(name=>{const input=fields.querySelector('[name="'+name+'"]');if(input)input.value=cast[name]||(name==='status'?'在籍':'');});}}
