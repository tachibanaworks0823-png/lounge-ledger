...
function calcDailyInput(input){
  ...
  // 手当はフリー・場内のバック入力として扱い、バック計へ一度だけ加算します。
  const back=backBreakdown.backTotal+allowance;
  ...
  return {hours,advance,back,allowance,hourly,gross,deductions,incomeTax:income,consumptionTax:consumption,welfare,baseDeduction,deduction,payout:Math.max(0,gross-deductions-advance),...backBreakdown,backTotal:back};
}
...
function applyPayrollAdjustment(castId,values){
  ...
  const hasManualAllowance=adjustment.allowance!==undefined&&adjustment.allowance!=='';
  const gross=hourly+back+(hasManualAllowance?allowance:0);
...
function enrichCastPayroll(castId,raw){
  ...
  const hasManualAllowance=adjustment.allowance!==undefined&&adjustment.allowance!=='';
  const gross=Number(result.hourly||0)+Number(result.back||0)+(hasManualAllowance?Number(result.allowance||0):0);
...
function calcCast(cast){
  ...
  if(dailyInputs.length){
    ...
    const back=backTotal,gross=hourly+back;
...
function renderCasts(){
  ...
  $('#castTable').innerHTML=sortedCasts().filter(visibleInPayroll).map(c=>{
    ...
    return '<tr><td><button type="button" class="payroll-cast-link" onclick="openForm(\'payrollDetails\',\''+c.id+'\')"><b>'+c.name+'</b><small>時給 '+yen(Number(c.hourly||0)+Number(x.hourlyBonus||0))+'</small></button></td><td>'+value(x.nominated)+'</td><td>'+count(x.area,'本')+' / '+count(x.main,'本')+' / '+count(x.companion,'本')+' / '+count(x.extension,'本')+'</td><td>'+count(x.days,'日')+'</td><td>'+Number(x.hours||0).toFixed(1)+'h</td><td>'+value(x.hourly)+'</td><td>'+value(x.companionBack)+'</td><td>'+value(x.mainBack)+'</td><td>'+value(x.extensionBack)+'</td><td>'+value(x.drink)+'</td><td>'+value(x.decoration)+'</td><td>'+value(x.bottleChampagne)+'</td><td>'+value(x.back)+'</td><td>'+value(x.consumptionTax)+'</td><td>'+value(x.incomeTax)+'</td><td>'+value(x.welfare)+'</td><td>'+value(x.pull)+'</td><td>'+value(x.deductionTotal)+'</td><td>'+value(x.advanceAmount)+'</td><td>'+value(x.gross)+'</td><td>'+value(x.payoutBeforeAdvance)+'</td><td><b>'+value(x.payout)+'</b></td><td>'+payRate+'</td><td>'+averageHourly+'</td><td><button class="text-button" onclick="editCastPayroll(\''+c.id+'\')">詳細・編集</button></td></tr>';
...
function renderSettings(){
  const rate=(key,label,value=data.settings[key]||0)=>'<label class="setting-field">'+label+'<input data-setting="'+key+'" type="number" min="0" value="'+value+'"></label>';
  const legacyDrinkRate=(key,legacyKey)=>data.settings[key]!==undefined&&data.settings[key]!==''?Number(data.settings[key]||0):Math.round(Number(data.settings[legacyKey]||0)/10);
  const labels={areaNomination:'場内指名バック（1本）',extension:'延長バック（1本）',mainNomination:'本指名バック（1本）',companion:'同伴バック（1本）'};
  const groups=[
    ['フリー・場内',[['freeDrinkRate','ドリンクバック（%）',legacyDrinkRate('freeDrinkRate','free1000')]]],
    ['本指名',[['mainDrinkRate','ドリンクバック（%）',legacyDrinkRate('mainDrinkRate','main1000')],['mainP','P'],['mainDecoration','飾り物'],['mainBottle','ボトル・シャンパン（%）']]],
    ['同伴',[['companionDrinkRate','ドリンクバック（%）',legacyDrinkRate('companionDrinkRate','companion1000')],['companionP','P'],['companionDecoration','飾り物'],['companionBottle','ボトル・シャンパン（%）']]]
  ];
  ...
  $('#backSettings').innerHTML='<div class="setting-rate-section full"><h3>基本バック</h3><div class="daily-rate-grid">'+Object.entries(labels).map(([k,l])=>rate(k,l)).join('')+'</div></div><div class="setting-rate-section daily-rate-section full">'+groups.map(([title,fields],index)=>'<section class="back-rate-group back-rate-group-'+index+'"><h4>'+title+'</h4><div class="daily-rate-grid">'+fields.map(([key,label,value])=>rate(key,label,value)).join('')+'</div></section>').join('')+'</div>';
...
const simulationDefaults = {
  ...
  targetSales:'',
  products:[{name:'',price:''}],
  system:{counterFee:'',boxFee:'',counterExtensionFee:'',boxExtensionFee:''},
  holidays:[]
};
let simulationDraft = [];
...
function syncSimulationForm(){
  ...
  sim.system={counterFee:get('#simCounterFee'),boxFee:get('#simBoxFee'),counterExtensionFee:get('#simCounterExtensionFee'),boxExtensionFee:get('#simBoxExtensionFee')};
...
function renderSimulation(){
  ...
  setValue('#simTargetSales',sim.targetSales);setValue('#simCounterFee',sim.system.counterFee);setValue('#simBoxFee',sim.system.boxFee);setValue('#simCounterExtensionFee',sim.system.counterExtensionFee);setValue('#simBoxExtensionFee',sim.system.boxExtensionFee);
...
function generateSimulationDraft(){
  ...
  const systemValues=[sim.system.counterFee,sim.system.boxFee,sim.system.counterExtensionFee,sim.system.boxExtensionFee].map(simulationMoney).filter(Boolean);
  ...
  const labels=pricedProducts.length?pricedProducts.map(item=>item.name||'商品'):[(sim.system.counterFee||sim.system.boxFee||sim.system.counterExtensionFee||sim.system.boxExtensionFee)?'50分セット':'シミュレーション'];
...
const batchBackDetail=()=>'<details class="batch-back-detail"><summary>バック打込み</summary>'+backGroups.map(([title,keys],index)=>{const count=index===1?'<label>本指名<input class="batch-main" type="number" min="0"></label>':index===2?'<label>同伴<input class="batch-companion" type="number" min="0"></label>':'';const allowance=index===0?'<label>手当<input class="batch-allowance" type="number" min="0"></label>':'';return '<section class="back-detail-group group-'+index+'"><h4>'+title+'</h4><div>'+count+keys.map(key=>'<label>'+backInputLabels[key]+'<input class="batch-'+key+'" type="number" min="0"></label>').join('')+allowance+'</div></section>';}).join('')+'</details>';
const dailyBackDetail=()=>'<details class="daily-back-detail full"><summary>バック打込み</summary>'+backGroups.map(([title,keys],index)=>{const count=index===1?'<label>本指名<input name="mainCount" type="number" min="0"></label>':index===2?'<label>同伴<input name="companionCount" type="number" min="0"></label>':'';const allowance=index===0?'<label>手当<input name="allowance" type="number" min="0"></label>':'';return '<section class="back-detail-group group-'+index+'"><h4>'+title+'</h4><div>'+count+keys.map(key=>'<label>'+backInputLabels[key]+'<input name="'+key+'" type="number" min="0"></label>').join('')+allowance+'</div></section>';}).join('')+'</details>';
...
function orderedCustomerNames(){
  return allCustomerNames().sort((a,b)=>{
    if(a==='新規')return b==='新規'?0:-1;
    if(b==='新規')return 1;
