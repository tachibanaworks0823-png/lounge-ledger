...
function renderSlips(){
  ...
  $('#slipTable').innerHTML=slips.slice().sort((a,b)=>a.date.localeCompare(b.date)*direction).map(s=>{
    ...
    return '<tr><td>'+dateJP(s.date)+'</td><td>'+ (s.customerName||'—')+'</td><td>'+s.id+'</td><td>'+yen(s.total)+'</td><td><span class="status '+(paymentMethodCategory(payment)==='card'?'':'cash')+'">'+payment+'</span></td><td>'+s.guests+'名</td><td><button class="text-button" onclick="removeItem(\'slips\',\''+s.id+'\')">削除</button></td></tr>';
...
function renderCastManagement(){
  const row=(c,showLeavingDate=false,showAddress=false)=>{const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';const address=[c.address,c.building].filter(Boolean).join(' ')||'—';const memo='<td>'+(c.memo||'—')+'</td>';const addressCells='<td><button class="text-button" onclick="copyCastAddress(\''+c.id+'\')">コピー</button></td><td>'+address+'</td>';return '<tr><td><b>'+c.name+'</b></td><td>'+castAge(c.birthday)+'</td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td>'+(showLeavingDate?'<td>'+(c.leavingDate?dateKey(c.leavingDate).replace(/-/g,'/'):'—')+'</td>':'')+(showAddress?addressCells+memo:memo)+'<td><button class="text-button" onclick="editCastProfile(\''+c.id+'\')">編集・詳細</button></td></tr>';};
...
window.selectCastListTab=tab=>{document.querySelectorAll('.cast-list-tab').forEach(button=>{const active=button.dataset.castListTab===tab;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active));});document.querySelectorAll('[data-cast-list-panel]').forEach(panel=>{const active=panel.dataset.castListPanel===tab;panel.hidden=!active;panel.classList.toggle('active',active);});const addCastButton=$('#addCastProfile');if(addCastButton){const show=tab==='active'||tab==='applications';addCastButton.hidden=!show;if(tab==='active'){addCastButton.textContent='＋ キャストを追加';addCastButton.onclick=()=>openForm('cast');}else if(tab==='applications'){addCastButton.textContent='＋ 応募を追加';addCastButton.onclick=()=>openForm('application');}}};
...
const dialog=$('#entryDialog'), form=$('#entryForm'), fields=$('#formFields'), mediaOrderDialog=$('#mediaOrderDialog'), mediaOrderList=$('#mediaOrderList');let mode='', slipDateSort='desc', dailyInputDateSort='desc', editingCastId=null, editingDailyInputId=null, editingApplicationId=null, mediaOrderDraft=[];
...
function openForm(type,castId=null){mode=type;form.autocomplete=(type==='cast'||type==='application')?'off':'on';fields.classList.toggle('cast-profile-fields',type==='cast');editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;editingApplicationId=type==='application'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#deleteApplicationButton').hidden=!(type==='application'&&editingApplicationId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':(type==='shift'||type==='shiftBatch'||type==='shopClosed')?'SHIFT':'CAST';
 if(type==='slip'){
   ...
   '<label class="field">決済<select required name="payment"><option value="" selected disabled>選択してください</option>'+paymentMethodOptions()+'<option value="__new__">＋ 決済方法を追加</option></select></label>'+
   ...
   '<label class="field"><span>指名</span><select required name="nominationType"><option value="" selected disabled>選択してください</option><option value="本指名">本指名</option><option value="同伴">同伴</option><option value="場内">場内</option></select></label>';bindCustomerHistoryButtons();bindPaymentMethodAdder();
 }
