...
function bindDesktopTimePickers(){fields.querySelectorAll('.desktop-time-picker').forEach(picker=>{const hour=picker.querySelector('.time-hour'),minute=picker.querySelector('.time-minute'),target=picker.querySelector('input[type=hidden]');const update=()=>target.value=hour.value&&minute.value?hour.value+':'+minute.value:'';hour.onchange=update;minute.onchange=update;});}
...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 if(type==='slip'){
 ...
 if(type==='dailyInput'){ $('#dialogTitle').textContent='日別打込み';fields.innerHTML=field('日付','date','date')+'<label class="field">キャスト<select name="castId">'+sortedCasts().map(c=>'<option value="'+c.id+'">'+c.name+'</option>').join('')+'</select></label>'+timeField('出勤','startTime')+timeField('退勤','endTime')+optionalField('実働時間','hours','number')+optionalField('日払い','advance','number')+optionalField('引き物','deduction','number')+optionalField('場内指名','areaNomination','number')+optionalField('本指名 本数','mainCount','number')+optionalField('同伴 本数','companionCount','number')+optionalField('本指名 売上','mainSales','number');}
 ...
 const now=new Date().toISOString().slice(0,10);if(!(type==='cast'&&editingCastId))fields.querySelectorAll('input[type=date]').forEach(x=>{if(!(type==='cast'&&(x.name==='leavingDate'||x.name==='birthday')))x.value=now;});if(type==='slip'||type==='dailyInput'){const dateField=fields.querySelector('input[name="date"]');if(dateField)dateField.value=businessDate();}if(type==='dailyInput')bindDesktopTimePickers();showEntryDialog();
}
['addSlip','dashboardAddSlip'].forEach(id=>$('#'+id).onclick=e=>{e.preventDefault();openForm('slip')});
$('#sortSlipsDate').onclick=()=>{slipDateSort=slipDateSort==='asc'?'desc':'asc';renderSlips()};$('#addDailyInput').onclick=()=>openForm('dailyInput');$('#addExpense').onclick=()=>openForm('expense');$('#addShift').onclick=()=>openForm('shift');$('#addShiftFromSchedule').onclick=()=>openForm('shift');$('#addCast').onclick=()=>openForm('cast');$('#addCastProfile').onclick=()=>openForm('cast');
form.addEventListener('submit',e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const x=Object.fromEntries(new FormData(form));if(mode==='slip'){data.slips.push({id:x.id,date:x.date,customerName:x.customerName,total:+x.total,card:x.payment==='カード'?+x.total:0,payment:x.payment,groups:1,guests:+x.guests,nominationType:x.nominationType,casts:[]})}if(mode==='dailyInput'){data.dailyInputs.push({id:'DI-'+Date.now(),date:x.date,castId:x.castId,startTime:x.startTime,endTime:x.endTime,hours:+x.hours,advance:+x.advance,deduction:+x.deduction,areaNomination:+x.areaNomination,mainCount:+x.mainCount,companionCount:+x.companionCount,mainSales:+x.mainSales})}if(mode==='expense'){const category=(x.newCategory||'').trim()||x.category;if((x.newCategory||'').trim()&&!data.settings.categories.includes(category))data.settings.categories.push(category);if(!data.settings.payeeHistory.includes(x.company))data.settings.payeeHistory.push(x.company);data.expenses.push({id:'E-'+Date.now(),date:x.date,category,company:x.company,note:x.note,amount:+x.amount})}if(mode==='shift'){data.shifts.push({date:x.date,castId:x.castId,hours:+x.hours,advance:+x.advance})}if(mode==='cast'){const profile={name:x.name,hourly:+x.hourly,status:x.status,joinedDate:x.joinedDate,leavingDate:x.leavingDate,phone:x.phone,birthday:x.birthday,memo:x.memo};const existing=data.casts.find(c=>c.id===editingCastId);if(existing)Object.assign(existing,profile);else data.casts.push({id:'c-'+Date.now(),...profile})}save();render();dialog.close();});
