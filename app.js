...
function dailyRows(){
  ...
  return Array.from({length:count},(_,i)=>{
    ...
    const slips=data.slips.filter(x=>x.date===date&&!isUnsettledSlip(x)), expenses=data.expenses.filter(x=>expensePostingDate(x)===date), shifts=data.shifts.filter(x=>x.date===date), dailyInputs=data.dailyInputs.filter(x=>x.date===date);
...
function slipTableRow(s){
  ...
  return '<tr class="'+(unsettled?'unsettled-slip':'')+'"><td>'+dateJP(s.date)+'</td><td>'+ (s.id||'—')+'</td><td>'+ (s.customerName||'—')+'</td><td>'+ (s.guests? s.guests+'名':'—')+'</td><td>'+yen(s.total)+'</td><td>'+(s.receipt?'<input class="receipt-list-check" type="checkbox" checked tabindex="-1" onclick="return false" aria-label="領収証あり">':'')+'</td><td><span class="status '+(unsettled?'unsettled-status ':slipPaymentCashOnly(s)?'cash':'')+'">'+paymentLabel+'</span></td><td><button class="text-button" onclick="editSlip('+slipIndex+')">編集</button></td></tr>';
}
...
function renderSlips(){
  ...
  $('#slipTable').innerHTML=slips.slice().sort((a,b)=>{const dateOrder=a.date.localeCompare(b.date)*direction;return dateOrder||String(a.id||'').localeCompare(String(b.id||''),'ja',{numeric:true})*direction;}).map(slipTableRow).join('')||empty(8,'伝票はまだありません');
}
...
function renderExpenses(){const m={},expenses=data.expenses.filter(x=>expenseAccountingMonth(x)===data.month);expenses.forEach(x=>m[x.category]=(m[x.category]||0)+Number(x.amount));const categoryNames=[...new Set([...(data.settings.categories||[]),...Object.keys(m).filter(Boolean)])];const total=expenses.reduce((sum,item)=>sum+Number(item.amount||0),0);$('#expenseSummary').innerHTML='<section class="expense-total-card"><p>支出合計</p><strong>'+yen(total)+'</strong><small>'+expenses.length+'件</small></section><section class="expense-category-list">'+categoryNames.map(category=>'<div class="expense-category-item'+(m[category]?'':' is-zero')+'"><span>'+category+'</span><b>'+yen(m[category]||0)+'</b></div>').join('')+'</section>';
$('#expenseTable').innerHTML=expenses.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(x=>`<tr><td>${dateJP(x.date)}</td><td>${yen(x.amount)}</td><td>${x.company}</td><td><span class="status">${x.category}</span></td><td>${x.note||'—'}</td><td><button class="text-button" onclick="editExpense('${x.id}')">編集</button></td></tr>`).join('')||empty(6,'支出はまだありません');}
...
function openForm(type,castId=null){mode=type;const primarySave=form.querySelector('.primary-button[value="save"]');primarySave.type='submit';primarySave.onclick=null;form.autocomplete=(type==='cast'||type==='application')?'off':'on';fields.classList.toggle('cast-profile-fields',type==='cast');editingCastId=type==='cast'?castId:null;editingDailyInputId=type==='dailyInput'?castId:null;editingApplicationId=type==='application'?castId:null;editingSlipIndex=type==='slip'&&castId!==null?Number(castId):null;editingExpenseId=type==='expense'?castId:null;form.querySelector('[value="save"]').hidden=type==='dailyDetails';$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);const castVisibilityButton=$('#toggleCastVisibilityButton');castVisibilityButton.hidden=!(type==='cast'&&editingCastId);if(type==='cast'&&editingCastId){const cast=data.casts.find(c=>c.id===editingCastId);castVisibilityButton.textContent=cast?.hidden?'表示に戻す':'非表示';}$('#deleteApplicationButton').hidden=!(type==='application'&&editingApplicationId);$('#deleteSlipButton').hidden=!(type==='slip'&&editingSlipIndex!==null);$('#deleteExpenseButton').hidden=!(type==='expense'&&editingExpenseId);$('#dialogKicker').textContent=type==='shift'||type==='shiftBatch'||type==='shopClosed'?'SHIFT':'';
 if(type==='slip'){
   ...
   '<div class="field slip-id-field"><span>伝票番号</span><div class="slip-number-rows" id="slipNumberRows"></div><button type="button" class="slip-number-add" onclick="addSlipNumberRow()">＋ 伝票番号を追加</button></div>'+
