...
function batchCastRow(castId=''){const casts=sortedCasts().filter(c=>effectiveCastStatus(c)!=='退店');const options=casts.map(c=>'<option value="'+c.id+'"'+(c.id===castId?' selected':'')+'>'+c.name+'</option>').join('');return '<article class="daily-batch-row" data-cast-id="'+castId+'"><div class="batch-cast-head"><label><small>キャスト</small><select class="batch-cast-select">'+options+'</select></label><button type="button" class="text-button remove-batch-cast">リセット</button></div><div class="batch-time-field batch-top-field"><small>出勤</small>'+batchTimePicker('start')+'</div><div class="batch-time-field batch-top-field"><small>退勤</small>'+batchTimePicker('end')+'</div><label class="batch-work-field batch-top-field"><small>実働</small><output class="batch-hours"></output></label><label class="batch-four-field"><small>場内</small><input class="batch-area" type="number"></label><label class="batch-four-field"><small>日払い</small><input class="batch-advance" type="number"></label><label class="batch-four-field"><small>手当</small><input class="batch-allowance" type="number"></label><label class="batch-four-field"><small>引き物</small><input class="batch-deduction" type="number"></label><label class="batch-sales-field"><small>本指名売上</small><input class="batch-sales" type="number"></label>'+batchBackDetail()+'</article>';}
