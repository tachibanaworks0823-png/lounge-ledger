...
function bindBatchRow(row){row.querySelectorAll('select').forEach(select=>select.onchange=()=>calculateBatchHours(row));row.querySelector('.remove-batch-cast').onclick=()=>{row.querySelectorAll('input').forEach(input=>input.value='');row.querySelectorAll('select:not(.batch-cast-select)').forEach(select=>select.value='');row.querySelector('.batch-hours').value='';};}
...
const batchBackDetail=()=>'<details class="batch-back-detail"><summary>バック打込み（フリー・場内／本指名／同伴）</summary>'+backGroups.map(([title,keys],index)=>'<section class="back-detail-group group-'+index+'"><h4>'+title+'</h4><div>'+keys.map(key=>'<label>'+backInputLabels[key]+'<input class="batch-'+key+'" type="number" min="0"></label>').join('')+'</div></section>').join('')+'</details>';
