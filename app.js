...
function renderCastManagement(){
  const row=(c,showLeavingDate=false,showAddress=false)=>{const checked=[c.termsSigned,c.photoSubmitted,c.residenceCertificate].filter(Boolean).length;const checkLabel=checked===3?'確認済':'未確認 '+checked+'/3';const address=[c.address,c.building].filter(Boolean).join(' ')||'—';const memo='<td>'+(c.memo||'—')+'</td>';const addressCells='<td><button class="text-button" onclick="copyCastAddress(&quot;'+c.id+'&quot;)">コピー</button></td><td>'+address+'</td>';const actions='<td><button class="text-button" onclick="editCastProfile(&quot;'+c.id+'&quot;)">編集・詳細</button><button class="text-button cast-visibility-button" onclick="toggleCastVisibility(&quot;'+c.id+'&quot;)">非表示</button></td>';return '<tr><td><b>'+c.name+'</b></td><td>'+castAge(c.birthday)+'</td><td><span class="cast-check-status '+(checked===3?'complete':'incomplete')+'">'+checkLabel+'</span></td>'+(showLeavingDate?'<td>'+(c.leavingDate?dateKey(c.leavingDate).replace(/-/g,'/'):'—')+'</td>':'')+(showAddress?addressCells+memo:memo)+actions+'</tr>';};
  const hiddenRow=c=>'<tr><td><b>'+c.name+'</b></td><td>'+effectiveCastStatus(c)+'</td><td>'+castAge(c.birthday)+'</td><td>'+(c.memo||'—')+'</td><td><button class="text-button" onclick="editCastProfile(&quot;'+c.id+'&quot;)">編集・詳細</button><button class="text-button cast-visibility-button" onclick="toggleCastVisibility(&quot;'+c.id+'&quot;)">表示に戻す</button></td></tr>';
...
const dialog=$('#entryDialog'), form=$('#entryForm'), fields=$('#formFields'), mediaOrderDialog=$('#mediaOrderDialog'), mediaOrderList=$('#mediaOrderList'), paymentMethodDialog=$('#paymentMethodDialog'), paymentMethodForm=$('#paymentMethodForm'), paymentMethodList=$('#paymentMethodList'), customerHistoryDialog=$('#customerHistoryDialog'), customerHistoryList=$('#customerHistoryList'), customerPickerDialog=$('#customerPickerDialog'), customerPickerList=$('#customerPickerList');let mode='', slipDateSort='desc', dailyInputDateSort='desc', editingCastId=null, editingDailyInputId=null, editingApplicationId=null, editingSlipIndex=null, mediaOrderDraft=[];
