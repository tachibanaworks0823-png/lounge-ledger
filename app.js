...
function openForm(type,castId=null){mode=type;editingCastId=type==='cast'?castId:null;$('#deleteCastButton').hidden=!(type==='cast'&&editingCastId);$('#dialogKicker').textContent=type==='slip'?'SALES SLIP':type==='expense'?'EXPENSE':type==='shift'?'SHIFT':'CAST';
 ...
 if(type==='dailyInput'){ $('#dialogTitle').textContent='日別打込み';fields.innerHTML=field('日付','date','date')+'<label class="field">キャスト<select name="castId">'+sortedCasts().map(c=>'<option value="'+c.id+'">'+c.name+'</option>').join('')+'</select></label>'+optionalField('出勤','startTime','time')+optionalField('退勤','endTime','time')+optionalField('実働時間','hours','number')+optionalField('日払い','advance','number')+optionalField('引き物','deduction','number')+optionalField('本指名 本数','mainCount','number')+optionalField('同伴 本数','companionCount','number')+optionalField('本指名 売上','mainSales','number');}
