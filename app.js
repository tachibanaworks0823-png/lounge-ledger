...
function renderExpenses(){const m={},expenses=data.expenses.filter(x=>expenseAccountingMonth(x)===data.month);expenses.forEach(x=>m[x.category]=(m[x.category]||0)+Number(x.amount));const categoryNames=[...new Set([...(data.settings.categories||[]),...Object.keys(m).filter(Boolean)])];const total=expenses.reduce((sum,item)=>sum+Number(item.amount||0),0),filteredExpenses=expenseCategoryFilter?expenses.filter(item=>item.category===expenseCategoryFilter):expenses;$('#expenseSummary').innerHTML='<button type="button" class="expense-total-card'+(!expenseCategoryFilter?' is-active':'')+'" onclick="filterExpenseCategory('')"><p>支出合計</p><strong>'+yen(total)+'</strong><small>'+expenses.length+'件 ・ すべて表示</small></button><section class="expense-category-list">'+categoryNames.map(category=>'<button type="button" class="expense-category-item'+(m[category]?'':' is-zero')+(expenseCategoryFilter===category?' is-active':'')+'" onclick="filterExpenseCategory(''+category.replace(/'/g,"\\'")+'')"><span>'+category+'</span><b>'+yen(m[category]||0)+'</b></button>').join('')+'</section>';const filterInfo=$('#expenseFilterInfo');if(filterInfo)filterInfo.textContent=expenseCategoryFilter?'「'+expenseCategoryFilter+'」の支出一覧':'カテゴリをクリックすると、そのカテゴリだけに絞り込めます。';$('#expenseTable').innerHTML=filteredExpenses.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(x=>`<tr><td>${dateJP(x.date)}</td><td>${yen(x.amount)}</td><td>${x.company}</td><td><span class="status">${x.category}</span></td><td>${x.note||'—'}</td><td><button class="text-button" onclick="editExpense('${x.id}')">編集</button></td></tr>`).join('')||empty(6,expenseCategoryFilter?'このカテゴリの支出はありません':'支出はまだありません');}
